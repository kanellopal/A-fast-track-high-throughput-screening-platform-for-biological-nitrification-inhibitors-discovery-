...
The μmax can be found in Supplementary Table S5.The μmax calculation is shown in Supplementary Figure S4E. The routine culture growth model curve is used in Supplementary Figure S3C  
...
Step 3: Prepare the boxplot (Figure S3A)

```{r echo=TRUE, message=FALSE, warning=FALSE}
library(ggplot2)
box_plot_DNA <- ggplot(data_1D_DNA, aes(x = Step, y = Cells)) + 
  geom_boxplot(aes(colour = Step), width = 5) + theme_light() + 
  labs(title = "N. communis Cell Density Determination", x = "Sample", y = "Cell Concentration (cells/mL)") +
...
Step 4: Prepare the lineplot (Figure S3B)  
...
line_plots_DNA <- ggplot(data_1D_DNA_lineplot, aes(x = factor(Step), y = Mean, group = Repetition)) + geom_point(aes(colour = Repetition)) + 
  theme_light() + 
  labs(title = "Nitromonas communis Cell Concentration Determination",subtitle = "Comparison between consecutive attempts", x = "Sample", y = "Cell Concentration (cells/mL)") + 
...
**Section 3 : N. communis fast-track cultures cell activity curves**

The activity rate can be found in Supplementary Table S5 and its calculation is shown in Supplementary Figure 4F. The fast-track culture activity plot is used in Supplementary Figure S3C  
...
ggsave("Figure S4F-NCOM fast-track activity rate.pdf", plot = line_plot_exponential_ln, device = "pdf", height = 10, width = 10)
