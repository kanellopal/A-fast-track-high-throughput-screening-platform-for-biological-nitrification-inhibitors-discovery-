---
title: "Nitrosomonas communis fast-track system standardisation"
...
**Section 1 : N. communis routine culture growth curve**

The μmax can be found in Supplementary Figure S4E and Supplementary Table S5. The routine culture growth model curve is used in Supplementary Figure S3C  
...
library(readxl)
data_routine <- read_excel("NCOM Standardisation.xlsx", sheet = 1, col_names = TRUE)
...
# Prepare the legend
legend_order <- c("1st", "2nd", "3rd", "4th", "6th_A", "6th_B", "7th_A", "7th_B", "8th", "9th")
...
ggsave("Figure S4E-NCOM maximum specific growth rate.pdf", plot = line_plot_exponential_ln, device = "pdf", height = 10, width = 10)

```


**Section 2 : N. communis fast-track cultures cell density data**

The boxplots produced can be found in Supplementary Figure S3A and Supplementary Figure S3B. Cell density data are found in Supplementary Table S4 
...
# We inport the data
library(dplyr)
data_1D_DNA <- read_excel("NCOM Standardisation.xlsx", sheet = 4, col_names = TRUE)
...
box_plot_DNA <- ggplot(data_1D_DNA, aes(x = Step, y = Cells)) + 
  ...
  labs(title = "N. ureae Cell Density Determination", x = "Sample", y = "Cell Concentration (cells/mL)") +
  ...
  scale_y_continuous(breaks = seq(1e+05, 4e+06, by = 6e+05), label = scales::scientific_format()) + 
  ...
  geom_hline(yintercept = 1.0e+05, linetype = "dashed") + 
...
line_plots_DNA <- ggplot(data_1D_DNA_lineplot, aes(x = factor(Step), y = Mean, group = Repetition)) + geom_point(aes(colour = Repetition)) + 
  theme_light() + labs(title = "Nitromonas ureae Cell Concentration Determination",subtitle = "Comparison between consecutive attempts", x = "Sample", y = "Cell Concentration (cells/mL)") + 
...
# Here, we create an R data frame named with the data
library(readxl)
data_1D_activity <- read_excel("NCOM Standardisation.xlsx", sheet = 5, col_names = TRUE)
# We can change the names of the columns to more R-friendly names
...
line_plot_merged <- ggplot(data_1D_activity, aes(x = Time, y = Nitrite)) + 
  ...
  labs(title = "Nitrosomonas communis  fast - track culture", subtitle = "Linear Regression compared to the growth model for the Routine cultures", x = "Time (Hours)", y = "[Nitrite] (uM)") + 
...
cairo_pdf("Figure S3. NCOM_Standardisation.pdf", height = 30, width = 30)
print(patchwork)
dev.off()
```
...
ggsave("Figure S4F-NUE maximum specific growth rate.pdf", plot = line_plot_exponential_ln, device = "pdf", height = 10, width = 10)
