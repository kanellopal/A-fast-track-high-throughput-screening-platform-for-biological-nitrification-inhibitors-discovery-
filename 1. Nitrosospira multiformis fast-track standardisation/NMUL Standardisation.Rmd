...
The μmax can be found in Supplementary Table S5. The μmax calculation is shown in Supplementary Figure S4A. The routine culture growth model curve is used in Supplementary Figure S1C  

Step 1: Data input.

```{r Data import and pre-processing, echo=TRUE}
# Here, we create an R data frame with the data
library(readxl)
...
Step 5: We can calculate the maximum specific growth rate from the activity data. Briefly, we isolate the data from the exponential growth phase (60h - 110h) and we perform a semi - logarithmic transformation, using the natural logarithm (ln) to transform the nitrite values 
...
The boxplots produced can be found in Supplementary Figures S1A and S1B. Cell density data are found in Supplementary Table S4 
...
Step 3: Prepare the boxplot (Figure S1A)
...
Step 4: Prepare the lineplot (Figure S1B)
...
The activity rate can be found in Supplementary Table S5 and its calculation is shown in Supplementary Figure 4B. The fast-track culture activity plot is used in Supplementary Figure S1C  
...
Step 3: Print the whole Figure (Supplementary Figure S1)
