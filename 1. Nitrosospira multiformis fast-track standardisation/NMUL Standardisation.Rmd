---
title: "Nitrosospira multiformis fast-track system standardisation"
...
**Section 1 : N. multiformis routine culture growth curve**

The μmax can be found in Supplementary Table S5. The routine culture growth model curve is used in Supplementary Figure S1C  
...
library(readxl)
data_routine <- read_excel("NMUL Standardisation.xlsx", sheet = 1, col_names = TRUE)
...
growth_sigmoid_model_plot <- growth_sigmoid_model_plot + 
  ...
  annotate("text", x = 150, y = 110, label = paste("I_max =", round(fitObj_sm$sigmoidalModel$maximum_Estimate, 2)), size = 6, hjust = 0) +
  annotate("text", x = 150, y = 60, label = paste("α_mid =", round(fitObj_sm$sigmoidalModel$slopeParam_Estimate, 3)), size = 6, hjust = 0) +
  annotate("text", x = 150, y = 10, label = paste("t_mid =", round(fitObj_sm$sigmoidalModel$midPoint_Estimate, 2)), size = 6, hjust = 0)
...
# Prepare the model equation
model_curve <- function(t) {I_max/(1+exp(-a*(t-t_mid)))}
# Prepare the legend
legend_order <- c("1st", "2nd", "3rd", "4th", "5th", "6th", "7th", "8th", "9th", 
...
Step 5: We can calculate the maximum specific growth rate from the activity data. Briefly, we isolate the data from the exponential growth phase (50h - 110h) and we perform a semi - logarithmic transformation, using the natural logarithm (ln) to transform the nitrite values 

```{r echo=TRUE, message=FALSE, warning=FALSE}
# We assume that the exponential growth phase lasts from 60 h - 110 h, so we isolate these data
library(dplyr)
data_exponential <- filter(data_routine, between(Time, 60, 110))
...
ggsave("Figure S4A-NMUL maximum specific growth rate.pdf", plot = line_plot_exponential_ln, device = "pdf", height = 10, width = 10)

```


**Section 2 : N. multiformis fast-track cultures cell density data**

The boxplots produced can be found in Supplementary Figure S1A and Supplementary Figure S1B. Cell density data are found in Supplementary Table S4 
...
# We inport the data
library(dplyr)
data_1D_DNA <- read_excel("NMUL Standardisation.xlsx", sheet = 4, col_names = TRUE)
...
```{r echo=TRUE, message=FALSE, warning=FALSE}
#### Statistical analysis on the DNA data ----
library(rstatix)
...
Step 3: Prepare the boxplot  

```{r echo=TRUE, message=FALSE, warning=FALSE}
library(ggplot2)
box_plot_DNA <- ggplot(data_1D_DNA, aes(x = Step, y = Cells)) + 
  geom_boxplot(aes(colour = Step), width = 5) + theme_light() + 
  labs(title = "N. multiformis Cell Density Determination", x = "Sample", y = "Cell Concentration (cells/mL)") +
  ...
  scale_y_continuous(breaks = seq(1e+07, 7e+08, by = 6e+07)) + 
...
line_plots_DNA <- ggplot(data_1D_DNA_lineplot, aes(x = factor(Step), y = Mean, group = Repetition)) + geom_point(aes(colour = Repetition)) + 
  theme_light() + labs(title = "Nitrosospira multiformis Cell Concentration Determination",subtitle = "Comparison between consecutive attempts", x = "Sample", y = "Cell Concentration (cells/mL)") + 
  theme(plot.title = element_blank(), axis.title = element_text(size = 40), axis.text = element_text(size = 36), legend.text = element_text(size = 32), legend.title = element_text(size = 36), plot.subtitle = element_blank(), axis.text.x=element_text(angle = 45, hjust = 1), legend.position = "right") + 
  scale_x_discrete(limits = legend_order_DNA) + 
...
**Section 3 : N. multiformis fast-track cultures cell activity curves**

The activity rate can be found in Supplementary Table S5. The fast-track culture activity plot is used in Supplementary Figure S1C  
...
# Here, we create an R data frame named with the data
library(readxl)
data_1D_activity <- read_excel("NMUL Standardisation.xlsx", sheet = 5, col_names = TRUE)
# We can change the names of the columns to more R-friendly names
...
legend_order_activity <- c("1st","2nd","3rd", "4th")
...
line_plot_merged <- ggplot(data_1D_activity, aes(x = Time, y = Nitrite)) + 
  ...
  labs(title = "Nitrosospira multiformis  fast - track culture", subtitle = "Linear Regression compared to the growth model for the Routine cultures", x = "Time (Hours)", y = "[Nitrite] (uM)") + 
...
cairo_pdf("Figure S2. NMUL_Standardisation.pdf", height = 30, width = 30)
print(patchwork)
dev.off()
```

Step 4: Determine the activity rate of the fast-track cultures in a similar way to the routine cultures μmax
...
line_plot_exponential_ln <- ggplot(data_1D_ln, aes(x = Time, y = Nitrite)) +
  ...
  scale_color_viridis(option = "D", discrete = T) +
  stat_smooth(method = "lm", col = "blue", se = TRUE, linewidth = 1, linetype = "dashed") + 
  geom_text(x = 6, y = 7, label = paste("Activity rate =", μmax, "±", μmax_std_error), colour = "blue", size = 6, hjust = 0) + 
  geom_text(x = 6, y = 6.88, label = paste("R-squared = ", round(linear_regression_report$adj.r.squared,2)), colour = "blue", size = 6, hjust = 0)

print(line_plot_exponential_ln)

ggsave("Figure S4B-NMUL fast-track activity rate.pdf", plot = line_plot_exponential_ln, device = "pdf", height = 10, width = 10)
