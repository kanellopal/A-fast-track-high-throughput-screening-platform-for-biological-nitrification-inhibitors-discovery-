---
title: "Ca. Nitrosocosmicus franklandianus fast-track system standardisation"
...
**Section 1 : "Ca. N. franklandianus routine culture growth curve and amoA abundance curve**

The routine culture growth model curve is used in Supplementary Figure S5Β 
...
library(readxl)
data_routine <- read_excel("NF Standardisation.xlsx", sheet = 1, col_names = TRUE)
...
cairo_pdf("Figure S5B-NF_nitrite_amoA_congruence.pdf", width = 10, height = 8)
...
plot(filter(data_routine, !is.na(Mean_Nitrite))$Time, filter(data_routine, !is.na(Mean_Nitrite))$Mean_Nitrite, pch=16, axes=FALSE, ylim=c(0,2100), xlab="", ylab="", 
     type="b",col="black", main="Candidatus Nitrosocosmicus franklandianus - Culture activity compared to flow cytometry data")
...
plot(filter(data_routine, !is.na(Mean_Nitrite))$Time, filter(data_routine, !is.na(Mean_Nitrite))$Mean_Cells,  xlab="", ylab="", ylim=c(4E+04,4E+06), 
...
**Section 2 : Ca. N. franklandianus fast-track cultures activity data**

The plot produced can be found at Supplementary Figure S5A
...
# Here, we create an R data frame named with the data
library(readxl)
data_1D <- read_excel("NF Standardisation.xlsx", sheet = 2, col_names = TRUE)
# We can change the names of the columns to more R-friendly names
...
```{r Data import and pre-processing, echo=TRUE}
# Create the plot 
line_plot_FTS_vs_routine_activity_NF <- ggplot(data_1D, aes(x = Time, y = Nitrite)) +
  ...
  theme_light() + 
  labs(title = "Candidatus Nitrosocosmicus franklandianus Fast - Track Culture", subtitle = "Linear Regression compared to the growth model for the Routine culture", x = "Time (Hours)", y = "[Nitrite] (μM)") + 
  scale_y_continuous(breaks = seq(from = 0, to = 2100, by = 300), limits = c(0,2100)) +
...
line_plot_FTS_vs_routine_activity_NF <- line_plot_FTS_vs_routine_activity_NF + 
...
print(line_plot_FTS_vs_routine_activity_NF)

cairo_pdf("Figure S5A-N. franklandianus activity vs routine.pdf", width = 24, height = 12, onefile = T)
print(line_plot_FTS_vs_routine_activity_NF)
dev.off()
